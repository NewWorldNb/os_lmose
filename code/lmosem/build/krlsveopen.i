...
op_dev_step:
    ...
    devp=krlonidfl_retn_device((void*)(&devid),1);
    if(devp==0)
    {
           return (-1);
    }
...
res_step:
    ...
    if(rethd!=(-1))
    {
        if(krlthd_del_objnode(tdp,rethd)==(-1))
        {
            hal_sysdie("open api td del objnode err");
        }
    }
    if(krldel_objnode(ondp)==0)
    {
...
sysstus_t krlsve_open_device(objnode_t* ondep)
{
    if(ondep->on_objadr==0)
    {
        return (-1);
    }
    if(krldev_io(ondep)==(-1))
    {
        return (-1);
    }
    return (0);
