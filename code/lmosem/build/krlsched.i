...
# 8 "../kernel/krlsched.c" 2

void thrdlst_t_init(thrdlst_t* initp)
{
    list_init(&initp->tdl_lsth);
    initp->tdl_curruntd=0;
    initp->tdl_nr=0;
    return;
}
...
void krlsched_wait(kwlst_t* wlst)
{
    ...
    {
        schdap->sda_thdlst[pity].tdl_curruntd=0;
    }
...
void krlsched_chkneed_pmptsched()
{
    ...
    if(schdap->sda_schdflgs==1&&schdap->sda_premptidx==0)
    {
        schdap->sda_schdflgs=0;
        schd=1;
    }
    hal_spinunlock_restflg_sti(&schdap->sda_lock,&cpuflg);
    if(schd==1)
...
thread_t* krlsched_select_thread()
{
    ...
    hal_spinlock_saveflg_cli(&schdap->sda_lock,&cufg);
    for(uint_t pity=0;pity<64;pity++)
    {
        if(schdap->sda_thdlst[pity].tdl_nr>0)
        {
            if(list_is_empty_careful(&(schdap->sda_thdlst[pity].tdl_lsth))==0)
            {
             tdtmp=((thread_t *)((char *)(schdap->sda_thdlst[pity].tdl_lsth.next)-(unsigned long)(&((thread_t *)0)->td_list)));
             list_del(&tdtmp->td_list);
             if(schdap->sda_thdlst[pity].tdl_curruntd!=0)
             {
