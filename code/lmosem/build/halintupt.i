...
void init_intfltdsc(mach_t* machp)
{
    ...
    for(uint_t i=(0+0); i<=(31+0);i++)
    {
        intfltdsc_t_init(&ifdp[i],1,0,i-0,i);
    }

    for(uint_t i=(0+32); i<=(14+32);i++)
    {
        intfltdsc_t_init(&ifdp[i],2,0,i-32,i);
    }

    for(uint_t i=(0+47); i<=(23+47);i++)
    {
        intfltdsc_t_init(&ifdp[i],3,0,i-47,i);
    }
...
uint_t hal_retn_intnr()
{
    return (uint_t)hal_io32_read(0x4a000014);
}
...
drvstus_t hal_clear_intpnd(uint_t bitnr)
{
     ...
     hal_io32_write(0x4a000010,inttmp);
     return (0);
}
...
drvstus_t hal_clear_srcpnd(uint_t ifdnr)
{
    ...
    if(flg==1)
    {
        ...
        inttmp=(1<<phylinenr);
        hal_io32_write(0x4a000000,inttmp);
    ...
    if(flg==2)
    {
        ...
        inttmp=(1<<phylinenr);
        hal_io32_write(0x4a000018,inttmp);
        return (0);
    }
    ...
    if(flg==3)
    {
        ...
        inttmp=(1<<phylinenr);
        hal_io32_write(0x560000a8,inttmp);
        return (0);
    }
...
drvstus_t hal_enable_intline(uint_t ifdnr)
{
    ...
    if(flg==1)
    {
        inttmp=hal_io32_read(0x4a000008);
        inttmp&=(~(1<<phylinenr));
        hal_io32_write(0x4a000008,inttmp);
        return (0);
    }
    if(flg==2)
    {

        inttmp=hal_io32_read(0x4a00001c);
        inttmp&=(~(1<<phylinenr));
        hal_io32_write(0x4a00001c,inttmp);
    ...
    if(flg==3)
    {
        ...
        inttmp=hal_io32_read(0x560000a4);
        inttmp&=(~(1<<phylinenr));
        hal_io32_write(0x560000a4,inttmp);
        return (0);
    }
