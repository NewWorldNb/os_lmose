...
void thread_t_init(thread_t* initp)
{
    ...
    initp->td_tick=0;
    initp->td_privilege=5;
    initp->td_priority=0;
    initp->td_runmode=0;
    initp->td_krlstktop=0;
    initp->td_krlstkstart=0;
    initp->td_usrstktop=0;
    initp->td_usrstkstart=0;
    ...
    for(uint_t hand=0;hand<8;hand++)
    {
        initp->td_handtbl[hand]=0;
    }
    return;
}
...
thread_t* krlnew_thread_core(void* filerun,uint_t flg,uint_t prilg,uint_t prity,size_t usrstksz,size_t krlstksz)
{
    ...
    if(krlstkadr==0)
    {
        if(krldelete(usrstkadr,usrstksz)==0)
        {
            return 0;
        }
        return 0;
    ...
    krlthreadkrlsatck_init(ret_td,filerun,0xd3,0x10);
...
thread_t* krlnew_thread(void* filerun,uint_t flg,uint_t prilg,uint_t prity,size_t usrstksz,size_t krlstksz)
{
    ...
    if((flg&0x1)!=0||filerun==0||usrstksz>0x8000||krlstksz>0x8000)
    {
        return 0;
    }

    if((prilg!=5&&prilg!=0)||(prity>64))
    {
        return 0;
    }
    if(usrstksz<0x8000)
