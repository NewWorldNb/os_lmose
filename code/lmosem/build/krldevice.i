...
void device_t_init(device_t* initp)
{
    ...
    initp->dev_drv=0;
    initp->dev_attrb=0;
    initp->dev_privdata=0;
    initp->dev_userdata=0;
    initp->dev_extdata=0;
    initp->dev_name=0;
...
void driver_t_init(driver_t* initp)
{
    ...
    initp->drv_safedsc=0;
    initp->drv_attrb=0;
    initp->drv_privdata=0;

    for(uint_t dsi=0;dsi<12;dsi++)
    {
...
drvstus_t krlrun_driverentry(drventyexit_t drventry)
{
    ...
    if(drventry(drvp,0,0)==(-1))
    {
        return (-1);
    }
    if(krldriver_add_system(drvp)==(-1))
    {
        return (-1);
    }
    return (0);
...
void init_krldriver()
{
    for(uint_t ei=0;osdrvetytabl[ei]!=0;ei++)
    {
        if(krlrun_driverentry(osdrvetytabl[ei])==(-1))
        {
...
drvstus_t del_driver_dsc(driver_t* drvp)
{
    if(krldelete((adr_t)drvp,sizeof(driver_t))==0)
    {
        return (-1);
    }
    return (0);
...
drvstus_t del_device_dsc(device_t* devp)
{
    if(krldelete((adr_t)devp,sizeof(device_t))==0)
    {
        return (-1);
    }
    return (0);
...
drvstus_t krldev_add_driver(device_t* devp,driver_t* drvp)
{
    ...
    for (lst = (&drvp->drv_alldevlist)->next; lst != (&drvp->drv_alldevlist); lst = lst->next)
    {
        fdevp=((device_t *)((char *)(lst)-(unsigned long)(&((device_t *)0)->dev_indrvlst)));
        if(krlcmp_devid(&devp->dev_id,&fdevp->dev_id)==1)
        {
            return (-1);
        }
    }
...
drvstus_t krlnew_device(device_t* devp)
{
    ...
    for (lstp = (&dtbp->devt_devclsl[devmty].dtl_list)->next; lstp != (&dtbp->devt_devclsl[devmty].dtl_list); lstp = lstp->next)
    {
        findevp=((device_t *)((char *)(lstp)-(unsigned long)(&((device_t *)0)->dev_intbllst)));
        if(krlcmp_devid(&devp->dev_id,&findevp->dev_id)==1)
        {
            rets=(-1);
            goto return_step;
        }
...
drvstus_t krldev_retn_request(device_t *devp,uint_t iocode,objnode_t** retreq)
{
    ...
    drvstus_t rets=(-1);
    hal_spinlock_saveflg_cli(&devp->dev_lock,&cpufg);
    for (list = (&devp->dev_rqlist)->next; list != (&devp->dev_rqlist); list = list->next)
    {
        np=((objnode_t *)((char *)(list)-(unsigned long)(&((objnode_t *)0)->on_list)));
...
device_t* krlonidfl_retn_device(void* dfname,uint_t flgs)
{
    ...
    {
        findevp=((device_t *)((char *)(lstp)-(unsigned long)(&((device_t *)0)->dev_intbllst)));
        if(krlcmp_devid(didp,&findevp->dev_id)==1)
        {
            findevp=findevp;
            goto return_step;
        }
...
drvstus_t krldev_io(objnode_t* nodep)
{
    ...
    if(nodep->on_objtype!=1||nodep->on_objadr==0)
    {
        return (-1);
    }
    if(nodep->on_opercode<0||nodep->on_opercode>=12)
    {
        return (-1);
    }
    return krldev_call_driver(devp,nodep->on_opercode,0,0,0,nodep);
}
