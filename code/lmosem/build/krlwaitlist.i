...
void krlwlst_up(kwlst_t* wlst)
{
    if(list_is_empty_careful(&wlst->wl_list)==1)
    {
        return;
    }
    krlsched_up(wlst);
...
void krlwlst_allup(kwlst_t* wlst)
{
    while(list_is_empty_careful(&wlst->wl_list)==0)
    {
        krlsched_up(wlst);
...
thread_t* krlwlst_del_thread(kwlst_t *wlst)
{
    ...
    list_h_t* list;
    hal_spinlock_saveflg_cli(&wlst->wl_lock,&cufg);
    for (list = (&wlst->wl_list)->next; list != (&wlst->wl_list); list = list->next)
    {
        tdp=((thread_t *)((char *)(list)-(unsigned long)(&((thread_t *)0)->td_list)));
        if(tdp==0||wlst->wl_tdnr<1)
        {
            tdp=0;
            goto return_step;
        }
