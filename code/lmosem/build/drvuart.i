...
# 8 "../drivers/drvuart.c" 2

void uart_set_driver(driver_t* drvp)
{
    drvp->drv_dipfun[0]=uart_open;
    drvp->drv_dipfun[1]=uart_close;
    drvp->drv_dipfun[2]=uart_read;
    drvp->drv_dipfun[3]=uart_write;
    drvp->drv_dipfun[4]=uart_lseek;
    drvp->drv_dipfun[5]=uart_ioctrl;
    drvp->drv_dipfun[6]=uart_dev_start;
    drvp->drv_dipfun[7]=uart_dev_stop;
    drvp->drv_dipfun[8]=uart_set_powerstus;
    drvp->drv_dipfun[9]=uart_enum_dev;
    drvp->drv_dipfun[10]=uart_flush;
    drvp->drv_dipfun[11]=uart_shutdown;
...
void init_devuart0()
{
    hal_io32_write(0x50000000,3);
    hal_io32_write(0x50000004,5);
    hal_io32_write(0x50000008,0);
    hal_io32_write(0x5000000C,3);
    hal_io32_write(0x50000028,26);
    return;
}
...
drvstus_t uart_entry(driver_t* drvp,uint_t val,void* p)
{
    ...
    if(krlnew_devhandle(devp,uart0_rxd_handle,(0 +32))==(-1))
    {

        return (-1);
    }

    if(krlnew_devhandle(devp,uart0_txd_handle,(1 +32))==(-1))
    {

        return (-1);
    }

    if(krlnew_devhandle(devp,uart0_err_handle,(2 +32))==(-1))
    {
...
drvstus_t uart_write(device_t* devp,void* iopack)
{
    ...
    {
        if(uartdev_send_char(devsn,p[bi])==(-1))
        {
            return (-1);
        }
    }
...
drvstus_t uartdev_send_char(uint_t uart,char_t ch)
{
    ...
    while(!(hal_io32_read(0x50000010)&4))
    {
        if(time>0x100000)
        {
            return (-1);
