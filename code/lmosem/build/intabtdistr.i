...
void hal_dbug_print_reg(intstkregs_t* intstkp)
{
    ...
    printfk("CSP_REG sp:%x INTPND:%x\n\r", hal_read_currmodesp(),hal_io32_read(0x4a000010));
...
void hal_prefabt_distr(void* sframe)
{
    ...
    hal_read_cp15regs(5),hal_read_cp15regs(6));
...
void hal_irq_distr(void* sframe)
{
    ...
    switch(intoset)
    {
        ...
        case 4:
            hal_eint_distr(sframe,intoset,4,8);
            break;
        case 5:
            hal_eint_distr(sframe,intoset,8,24);
            break;
        case 6:
            hal_sint_distr(sframe,intoset,11,13);
            break;
        case 9:
            hal_sint_distr(sframe,intoset,13,15);
            break;
        case 15:
            hal_sint_distr(sframe,intoset,6,9);
            break;

        case 23:
            hal_sint_distr(sframe,intoset,3,6);
            break;
        case 28:
            hal_sint_distr(sframe,intoset,0,3);
            break;
        case 31:
            hal_sint_distr(sframe,intoset,9,11);
            break;
        default:
...
void hal_eint_distr(void* sframe,uint_t mintnr,uint_t pndbts,uint_t pndbte)
{
    u32_t pnd=hal_io32_read(0x560000a8);
    pnd&=0xffffff;
    ...
    for(uint_t bi=pndbts;bi<pndbte;bi++)
    {
        if(((pnd>>bi)&1)==1)
        {
            hal_run_intflthandle((bi+47),sframe);
            hal_clear_srcpnd((bi+47));
        }
    }
    return;
}
...
void hal_sint_distr(void* sframe,uint_t mintnr,uint_t pndbts,uint_t pndbte)
{
    u32_t pnd=hal_io32_read(0x4a000018);
    pnd&=0x7fff;
    ...
    for(uint_t bi=pndbts;bi<pndbte;bi++)
    {
        if(((pnd>>bi)&1)==1)
        {
            hal_run_intflthandle((bi+32),sframe);
            hal_clear_srcpnd((bi+32));
        }
    }
    return;
}
...
void hal_lcdint_distr(void* sframe,uint_t mintnr,uint_t pndbts,uint_t pndbte)
{
    u32_t pnd=hal_io32_read(0x4d000054);
    pnd&=0x3;
...
void hal_int_distr(void* sframe,uint_t mintnr)
{
    ...
    hal_run_intflthandle((mintnr+0),sframe);
    hal_clear_srcpnd((mintnr+0));
    return;
}
